// Landing v1 §4/§8: the sample data module validates against the payload rules, and the Express
...
import { clientPayload, faqPayload, portfolioCaseStudyPayload, testimonialPayload } from "../shared/content.js";
...
test("sample data: counts, fixed sample- ids and sample: true everywhere", () => {
  ...
  const records = [...SAMPLE_FAQS, ...SAMPLE_TESTIMONIALS, ...SAMPLE_CLIENTS];
...
test("sample data passes the create payload rules unchanged", () => {
  for (const [items, build] of [[SAMPLE_FAQS, faqPayload], [SAMPLE_TESTIMONIALS, testimonialPayload], [SAMPLE_CLIENTS, clientPayload]]) {
...
test("Express seed: dry run on a temp JSON store is idempotent and keeps admin edits", async () => {
  ...
  assert.deepEqual([first.created, first.updated, first.kept, first.portfolio], [20, 0, 0, 15]);
  ...
  assert.deepEqual([second.created, second.updated, second.kept, second.portfolio], [0, 20, 0, 15]);
  assert.equal((await store.listCollection("faqs", { includeInactive: true })).length, 8);

  // An admin saves one FAQ and the website section: a re-seed keeps both.
