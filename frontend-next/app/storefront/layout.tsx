 ...
 * `overflow-x-clip` keeps side-sliding entrances from ever widening the page; unlike `hidden` it leaves sticky elements working.
 */
export default async function StorefrontLayout({ children }: LayoutProps<"/storefront">) {
  ...
  return (
    <div className="flex min-h-screen flex-1 flex-col overflow-x-clip bg-slate-50 font-sans text-slate-900 antialiased">
      ...
      <StoreHeader phone={settings.business.phone} calculatorEnabled={Boolean(settings.calculator)} productsEnabled={settings.website.productsEnabled} />
