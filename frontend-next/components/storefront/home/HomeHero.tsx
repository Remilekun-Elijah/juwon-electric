...
/** Home hero on the brand panel: the page's h1, CTAs, battery-type shortcuts, a photo with the starting price, and trust points. */
export default function HomeHero({ phone, fromPrice, packageTypes = [] }: HomeHeroProps) {
  ...
  return (
    <div className={cn(storeContainer, "pt-4 sm:pt-6 lg:pt-8")}>
      <BrandPanel as="section" aria-labelledby="home-hero-heading" className={cn(storeFadeUp, "px-5 py-10 sm:px-10 sm:py-12 lg:px-14")}>
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)] lg:gap-14">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-100">Inverter, battery and solar systems</p>
            <h1
              id="home-hero-heading"
              className="mt-3 text-3xl font-semibold leading-[1.08] tracking-tight text-balance sm:text-4xl lg:text-[3.25rem]"
            >
            ...
            <ul className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm text-brand-50/90">
            ...
            {packageTypes.length > 0 && (
              <nav aria-label="Shop packages by battery type" className="mt-8 border-t border-white/10 pt-6">
          ...
          <div className="relative min-w-0">
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-elev-5">
              <Image
                src="/panel-3.webp"
            ...
            {typeof fromPrice === "number" && fromPrice > 0 && (
              <div className="mt-3 flex items-center justify-between gap-3 rounded-xl bg-white p-4 text-slate-900 shadow-elev-4 sm:absolute sm:bottom-6 sm:left-6 sm:mt-0 sm:min-w-[260px]">
                <div className="min-w-0">
