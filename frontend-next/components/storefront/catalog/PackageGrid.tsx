...
export default function PackageGrid({ packages, headingAs = "h3", compact = false, className }: PackageGridProps) {
  return (
    <ul className={cn("grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3", className)}>
      {packages.map((pkg) => (
        <li key={packageKey(pkg)}>
          <PackageCard pkg={pkg} headingAs={headingAs} compact={compact} />
        </li>
      ))}
    </ul>
  );
