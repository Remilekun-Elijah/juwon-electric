        ...
        {segments.map((segment, index) => {
          ...
          return (
            <li key={contentKey(segment, index)} className="min-w-0">
              <article className={cn(storeCard, "group relative flex h-full overflow-hidden transition-shadow hover:shadow-elev-3")}>
                <ContentImage
                  src={segment.image}
                  alt=""
                  sizes="(min-width: 640px) 140px, 112px"
                  className="w-28 shrink-0 self-stretch border-r border-slate-200 sm:w-36"
                  imageClassName="transition-transform duration-300 motion-safe:group-hover:scale-[1.03]"
                />
