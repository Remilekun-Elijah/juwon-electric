...
import { storeArrowNudge, storeContainer, storeEyebrowOnSurface, storePress, storeSection } from "@/lib/storefront/styles";
...
/**
 * "Size your system" teaser linking to /calculator (LANDING_V1 §7.7) on a `surface` red panel with gold accents
  ...
  return (
    <section aria-labelledby={headingId} className={storeSection}>
      <Reveal className={storeContainer}>
        <BrandPanel className="bg-surface px-5 py-10 sm:px-10 sm:py-12 lg:px-14">
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] lg:items-center">
            <div className="min-w-0">
              <p className={storeEyebrowOnSurface}>Size your system</p>
              ...
              <Link
                ...
                className={cn(
                  ...
                  "focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-gold-300 focus-visible:ring-offset-2 focus-visible:ring-offset-surface",
                  storePress
                )}
              >
            ...
            <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm sm:p-6">
              ...
              <Reveal as="ul" stagger delay={150} className="mt-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                {outputs.map(({ icon: Icon, label, value }) => (
                  <li key={label} className="flex items-center gap-3 rounded-xl border border-white/10 bg-surface/40 p-4">
              ...
              {hasExample && <p className="mt-3 text-xs leading-relaxed text-white">
