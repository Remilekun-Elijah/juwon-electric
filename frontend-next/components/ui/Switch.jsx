...
export function Switch({ checked, onChange, label, description, disabled, name, className, ...props }) {
  const toggle = (
    ...
    >
      <span
        aria-hidden="true"
        className="pointer-events-none inline-block h-4 w-4 translate-x-0 rounded-full bg-white shadow-sm transition group-data-[checked]:translate-x-4"
      />
    </HeadlessSwitch>
  );
