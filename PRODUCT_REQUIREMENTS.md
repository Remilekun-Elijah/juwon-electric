...
- Admin console chrome (2026-09-17): the admin sidebar (desktop rail and mobile drawer) and the top bar are the storefront's brand red `brand-950` (2026-09-23: the logo red `#d42027`, ~~`#3a090b`, which read as brown~~) with a hairline `white/10` border. Sidebar menu links, group headings and **Sign out** are gold, the current page is white on a translucent pill with a gold edge marker, and count badges are gold with dark text. The top bar uses white text with glass (translucent white) refresh and menu buttons and a gold avatar; the account dropdown panel and all page content, cards and tables stay light. Text keeps at least 4.5:1 contrast (2026-09-23: sidebar text is pale gold `gold-100` or white on the logo red) and focus rings are gold on the dark chrome.
...
- Colour: a gold accent scale taken from the Juwon Electric logo. Gold is used only as an accent on the dark surfaces, which are the brand red brand-950 (2026-09-23: brand red and gold are the two logo colours, red `#d42027` for buttons and dark surfaces and gold `#e8c557` / `#f5e285`, ~~coral `#db464c`, maroon and near-black brown, and gold `#dfc638`~~; revised 2026-09-17: the storefront's dark bands, hero overlays, header glass and footer were near-black slate; they are now brand red): highlighted headline words, stat numbers and primary buttons on dark. Text on gold buttons is dark (slate-950). Body text keeps a contrast of at least 4.5:1.
- Header:
...
- Hero (full-bleed):
  ...
  - Content: the headline "Reliable Energy / Solutions for / **Homes & Businesses**" on three lines, the last in a gold gradient (owner copy, 2026-09-18). Each line is clipped for its entrance animation, so the copy is split where it should break and the heading is capped at 22 characters wide, each line rising into view; the lead paragraph; a gold **View Packages** button and a glass **Chat on WhatsApp** button (when a WhatsApp number is set; otherwise **Talk to an engineer**, which calls); under the buttons, a status line "Solar & energy solutions across Nigeria" with a softly pulsing green dot (2026-09-23: ~~a glass pill above the headline~~).
  ...
  - On load the headline lines, lead, buttons, status line, stats and price card appear one after another (about 100 ms apart). All of it is in the page HTML.
...
- Section rhythm (no two neighbouring sections share a white background), in page order:
  ...
  15. Final call to action: brand red panel over an installation photo, eyebrow "Ready when you are", a gold **View Packages** button, **Call**, and **Chat on WhatsApp** when set, followed by the business phone numbers, email, address and opening hours (the home page has no separate contact band).
...
15. Final call to action: View Packages, Call, and WhatsApp (only when a WhatsApp number is set), then the business phone numbers, email, address and opening hours.
...
2026-09-23 (one colour file; labels, footer and admin on the red)
- Colours now live in one file, `frontend-next/app/theme.css` (imported by `globals.css`), shared by the storefront and the admin. It starts with two roles, `--color-surface` (`#d42027`, the red background: dark sections, page intros, header bar, hero wash, calculator and call-to-action panels, footer, admin sidebar, top bar and login panel) and `--color-surface-label` (`#fef6d6`, small text on the red), then the logo's red and gold palettes. Every `bg-/from-/via-/to-/ring-offset-brand-950`, `-900` and `-800` background became `surface`, and `gold-100` became `surface-label`, so a value changed in `theme.css` reaches both apps. `brand-900`/`950` are dark reds again (`#8a151a`, `#5c0e11`) for text on light tints. Not driven by the file: `themeColor` in `app/layout.tsx` and the red in `quote.svg`, `circular.svg`, `getInTouch.svg`.
- §6.10: eyebrows on the red (page intros such as "Our People", "Join Our Team", "Our Services", "Catalogue"; dark sections; "Ready when you are", "Size your system", "Talk to us"; the admin login label) are a solid gold chip with dark text (`storeEyebrowOnSurface`, 12:1), since 12 px pale gold still read as faint where the intro photo shows through. Footer column titles are bold white with a gold rule; the hero status line is white.
- §6.10: the footer **Subscribe** button is gold with dark text on the red (it was brand red and disappeared into the footer); its errors are pale gold instead of `red-300`.
- Admin chrome: the top bar is solid `surface` (it was `surface/80` over a blur, which washed out to pink over the light page); the login side panel is `surface` (was `brand-800`); sidebar group headings are bold.

2026-09-23 (readable text on the red)
- §6.10: only white (5.2:1) and a new pale gold `gold-100` `#fef6d6` (4.8:1; 4.6:1 on the `white/5` cards) reach 4.5:1 on the logo red, so small text on the red surfaces now uses one of the two. Descriptions, body copy, links, stat labels, form notes, breadcrumbs' current page and the footer (was `white/50`–`white/90`, `brand-50/70`–`/90`, `brand-100`) are white; section eyebrows, footer column headings, business-detail labels, breadcrumb links, the hero status line and the admin sidebar's links, group headings, brand chip and **Sign out** (were `gold-300`/`gold-400`, `brand-100`/`200` or `white/50`) are `gold-100`. Gold `gold-400` stays for large text (headline accent, stat numbers, step numbers, prices, at least 24 px) and icons, where 3:1 applies. Footer and final call-to-action links hover to `gold-100` with an underline instead of `gold-300`.

2026-09-23 (the logo's red and gold)
- §6.10 colour: the brand scales now use the two colours measured from the logo (`public/logo.svg`): the bolt red `#d42027` and the gold `#e8c557` (lettering) / `#f5e285` (bulb). `brand-500`/`600` (buttons, links) and `brand-900`/`950` (dark sections, header, hero wash, calculator panel, final call to action, footer, admin chrome) are `#d42027`; `brand-700` `#b91c22` and `brand-800` `#a3191e` are deeper steps for hover and small text. `gold-300` `#f5e285`, `gold-400` `#e8c557`, `gold-500` `#d9b243`, `gold-600` `#b8932a` replace the greener `#ecd873` / `#dfc638` / `#c9ad1f` / `#a48c16`. The classic site's `navbar_color`, `deep_red` and `--gold`, the scrollbar, the browser `theme-color`, the vacancies editor border, the Why choose us hover glow and the `quote`, `circular` and `getInTouch` illustrations follow. This replaces the earlier 2026-09-23 reds (`#c8373d` / `#bf3339` / `#b82f35`).
- Contrast: white on `#d42027` and `#d42027` on white are both 5.2:1, and slate-950 on gold is 12:1. Gold on the red is 3.1:1 (`gold-400`) and 4.0:1 (`gold-300`), and `white/70` muted text on the red is 3.2:1: fine for large text and icons, under 4.5:1 for small text, so small text on the red was changed (next entry).

2026-09-23 (brand red instead of brown, industries, hero status line, client logos)
- §6.10 colour: `brand-700` `#94161b` → `#c8373d`, `brand-800` `#811418` → `#bf3339`, `brand-900` `#5e0f12` → `#b82f35`, `brand-950` `#3a090b` → `#b82f35`; the classic site's `navbar_color` and `deep_red`, the browser `theme-color` and `public/quote.svg` follow. The owner asked for the brand's red instead of the brown dark bands, hero wash, footer, admin chrome and small headings. The steps stay a little deeper than `#db464c` for text: `brand-700` on white is 5.2:1 and white on `brand-950` is 6.0:1 (white on `#db464c` would be 4.2:1). Known gap: small gold text on the dark surfaces (gold-400 3.5:1, gold-300 4.2:1) and `white/70` muted text (3.7:1) are now under 4.5:1; to review.
- §6.10 Industries We Serve: **Manufacturing & Industrial** reads **Manufacturing & Industrial Facilities**, and **Infrastructure** is removed (seven cards).
- §6.10 home hero: the "Solar & energy solutions across Nigeria" line moved from a glass pill above the headline to a status line under the buttons, so the headline opens the hero.
- §6.10 client logos: the second copy of the logo marquee is no longer `inert` (still `aria-hidden`, links `tabIndex=-1`). `inert` blocked the pointer, so logos that had looped round stayed grey on hover and could not be clicked.
- Real client logo files for the owner's clients (GIG Logistics, Olowu of Owu palace, Nepal Energies, Soundcity, RCCG, Olsale Farms, Assemblies of God Nigeria, Superb Mart & Bakery) are in `frontend-next/public/clients/`, taken from each client's own website or Instagram, for use as `/clients/<name>.png` in **Website → Client logos**. The Superb Mart file is low resolution and should be replaced with the client's original.
...
- **Our work is no longer grouped or filtered by industry.** `/portfolio` shows one gallery (no `?category=` chips), the home/segment "See our work" links are gone, and the admin loses the **Customer Segments** manager (Services page) and the **Category** field on the portfolio form. `customerSegments` is no longer seeded (fresh installs start with none); the collection and its admin/services endpoints stay registered and simply return an empty list, and the portfolio `category` field remains accepted but unused — a full backend removal is a deferred post-handover cleanup. This supersedes the portfolio category filter (§6.11) and the "Industries We Power" segment cards below.

2026-09-20 (real installation photos site-wide)
- The seeded stock imagery (home hero slideshow, every page-intro background, customer-segment cards, the portfolio gallery, the home header and the contact background) is replaced with the owner's own inverter-and-battery installation photos. The 25 supplied phone photos (HEIC/JPG, 47 MB) were converted to web formats, auto-rotated upright, resized to 1400–1600px and recompressed to ~3 MB total, then written over the existing asset files so no records or references had to change. Portfolio records now point at their real `.jpg` paths (was `.svg`, which the content-image frame letterboxed with `object-contain`); as `.jpg` they fill the frame with `object-cover`. Hero and admin-login alt text that described "rooftop solar panels" was corrected to describe inverter/battery installations. The production D1 portfolio rows still carry the old `.svg` paths and need the same `.svg`→`.jpg` update at deploy time to fill correctly on the live site.

2026-09-20 (dependency audit, Vite-site retirement prep, 404 and doc fixes)
- Dependencies: `npm audit` was run across all four package roots. The production Worker was already clean. The Express backend's 10 advisories (including the "critical" mongoose and "high" express, none reachable in our code) were patched — `npm audit fix` plus nodemailer 10 — leaving it at **0** with tests still 90/90. `frontend-next` has one unfixable low (`quill` HTML-export XSS, admin-only, mitigated by the sanitiser + CSP), documented as an accepted exception. CI now runs `npm audit --audit-level=high --omit=dev` on backend, worker and frontend-next, so a new high/critical CVE fails the build.
- Legacy Vite site (`frontend/`): confirmed retirable — `frontend-next` is a complete superset of its public pages, and its own advisories (vite/esbuild dev-server-only, react-router) are not reachable in the static build. The Express backend was decoupled from it (it now owns `backend/data/plans.json` instead of reading `frontend/src/utils/plans.json`). Removing the folder and repointing the production domain to `frontend-next` is pending (a Vercel cut-over on the owner's side; see docs/DEPLOYMENT.md).
- Storefront 404: the suggestion grid now offers four escape routes (Packages, Products, Our services, Contact us; Products still hidden when the products area is off) instead of three.
- Docs: corrected stale settings-routing guidance (the removed "Back to settings"/overview step), the top-nav list (Careers now shows on desktop, not only on phones), and the remaining "Shop packages" / package-filter "by type" wording (now **View Packages** / by **category**).
...
- Content (live): four sample vacancies were published for review — Solar Installation Engineer, Technical Sales Representative and Inventory Officer as open roles, and Customer Support Officer as a draft. Each carries a hidden `sample` marker so they are easy to find and replace.
