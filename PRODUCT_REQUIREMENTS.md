...
- **Our work is no longer grouped or filtered by industry.** `/portfolio` shows one gallery (no `?category=` chips), the home/segment "See our work" links are gone, and the admin loses the **Customer Segments** manager (Services page) and the **Category** field on the portfolio form. The portfolio `category` field and the `customerSegments` API/collection remain in the backend but are dormant (a later cleanup can remove them). This supersedes the portfolio category filter (§6.11) and the "Industries We Power" segment cards below.
